...
1. **Add-on & Affiliate Revenue** — Spa/Dental affiliate (`ADD_ON_SERVICES.md`) + OTA experiences (`OTA_INTEGRATIONS.md`) để bổ sung doanh thu khi traffic ổn định.
2. **Thanh toán trực tuyến** (Stripe quốc tế + VNPay/MoMo nội địa) — chạy **song song** với Book Now – Pay Later, không thay thế.
3. **Đa ngôn ngữ thật sự** (i18n) — Pháp, Đức, Hàn, Nhật cho các thị trường lớn nhất.
4. **Reviews & UGC** — user upload ảnh chuyến đi, hashtag campaign, repost trên Instagram.
5. **Content engine** — blog/travel guide chất lượng cao, Related Posts tự động, CTA mạnh về Booking.
6. **Admin Analytics nâng cao** — doanh thu theo thị trường nguồn, theo mùa, alert peak season, tỷ lệ Self vs Partner.
7. **Personalization** — gợi ý tour theo quốc tịch / mùa / lịch sử xem.
8. **Auto social posting** — tour/blog mới tự đăng lên Facebook + Instagram.
...
2. **Modular > tối ưu sớm.** Đặc biệt với booking và payment — code phải sẵn sàng cho Phase 4.
