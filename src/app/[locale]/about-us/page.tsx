...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: localizedUrl(getSiteUrl(), locale, "/about-us"), languages: buildAlternates(getSiteUrl(), "/about-us") }
  };
}
...
export default async function AboutUsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const [team, settings] = await Promise.all([getTeamMembers(8), getSiteSettings(locale)]);
