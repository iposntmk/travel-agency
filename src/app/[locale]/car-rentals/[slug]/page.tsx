...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const rental = (await getCarRentalsForList({ limit: 100, locale })).find((item) => item.slug === slug);
  const path = `/car-rentals/${slug}`;
  return rental
    ? { title: rental.title, alternates: { canonical: localizedUrl(getSiteUrl(), locale, path), languages: buildAlternates(getSiteUrl(), path) } }
