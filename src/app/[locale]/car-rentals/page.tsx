...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: localizedUrl(getSiteUrl(), locale, "/car-rentals"),
      languages: buildAlternates(getSiteUrl(), "/car-rentals")
    }
  };
}
...
export default async function CarRentalsPage({ params, searchParams }: PageProps) {
  const { locale } = await params;
  setRequestLocale(locale);
  const sp = (await searchParams) ?? {};
  const [rentals, destinations] = await Promise.all([
    getCarRentalsForList({
      destinationSlug: sp.destination,
      vehicleType: sp.vehicle,
      route: sp.route,
      limit: 48,
      locale
    }),
