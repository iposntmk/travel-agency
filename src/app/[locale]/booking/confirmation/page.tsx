...
import { bookingConfirmationJsonLd, breadcrumbJsonLd } from "@/lib/structured-data";
...
export default async function ConfirmationPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const pageUrl = localizedUrl(siteUrl, locale, "/booking/confirmation");
  const [upsellTours, siteSettings] = await Promise.all([
    getToursForList({ featuredOnly: true, limit: 3, locale }).then((tours) =>
      tours.length > 0 ? tours : getToursForList({ limit: 3, locale })
    ),
